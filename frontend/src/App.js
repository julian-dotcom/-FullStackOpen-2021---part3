...
const App = () => {
  ...
  const removeEntry = (val) => {
    console.log(val)
    let toRemove = persons.find(person => person.id === val)
    if (!toRemove) {return} // Quit function in case toRemove isn't found.
    console.log(toRemove)
    if(!window.confirm(`Delete person ${toRemove.name}`)) {return} //check if user agrees to actually delete entry
    personsService
      .remove(toRemove.id)
      .then(response => hook())
